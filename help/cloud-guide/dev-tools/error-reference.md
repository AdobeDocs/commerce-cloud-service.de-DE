...
Kritische Fehler weisen auf ein Problem mit der Adobe Commerce in der Cloud-Infrastruktur-Projektkonfiguration hin, das zu einem Implementierungsfehler führt, z. B. zu einer falschen, nicht unterstützten oder fehlenden Konfiguration für die erforderlichen Einstellungen. Vor der Bereitstellung müssen Sie die Konfiguration aktualisieren, um diese Fehler zu beheben.

### Build-Phase

| Fehler-Code | Schritt erstellen | Fehlerbeschreibung (Titel) | Vorgeschlagene Aktion |
| - | - | - | - |
| 2 |  | Schreiben in `./app/etc/env.php` file | Das Bereitstellungsskript kann die erforderlichen Änderungen nicht vornehmen `/app/etc/env.php` -Datei. Überprüfen Sie Ihre Dateisystemberechtigungen. |
| 3 |  | Die Konfiguration wird nicht im `schema.yaml` file | Die Konfiguration wird nicht im `./vendor/magento/ece-tools/config/schema.yaml` -Datei. Überprüfen Sie, ob der Name der Konfigurationsvariablen korrekt ist und ob er definiert ist. |
...
| 7 | refresh-modules | Schreiben in `./app/etc/config.php` file | Das Bereitstellungsskript kann keine erforderlichen Änderungen an der `/app/etc/config.php` -Datei. Überprüfen Sie Ihre Dateisystemberechtigungen. |
| 8 | validate-config | Die `composer.json` file | Die `./composer.json` -Datei. Überprüfen Sie die Dateiberechtigungen. |
| 9 | validate-config | Composer.json fehlt der erforderliche Abschnitt zum automatischen Laden | Erforderlich `autoload` -Abschnitt fehlt im `composer.json` -Datei. Vergleichen Sie den Abschnitt &quot;Autoload&quot;mit dem Abschnitt `composer.json` in der Cloud-Vorlage und fügen Sie die fehlende Konfiguration hinzu. |
| 10 | validate-config | Die Datei `.magento.env.yaml` enthält eine Option, die nicht im Schema deklariert ist, oder eine Option, die mit einem ungültigen Wert oder einer ungültigen Phase konfiguriert ist | Die `./.magento.env.yaml` -Datei enthält eine ungültige Konfiguration. Detaillierte Informationen finden Sie im Fehlerprotokoll . |
| 11 | refresh-modules | Befehl fehlgeschlagen: `/bin/magento module:enable --all` | Ausführen versuchen `composer update` lokal. Übertragen Sie dann die aktualisierte `composer.lock` -Datei. Überprüfen Sie auch die `cloud.log` für weitere Informationen. Für eine detailliertere Befehlsausgabe fügen Sie die `VERBOSE_COMMANDS: '-vvv'` -Option `.magento.env.yaml` -Datei. |
| 12 | apply-patches | Patch nicht angewendet |  |
| 13 | set-report-dir-nesting-level | Schreiben in die Datei nicht möglich `/pub/errors/local.xml` |  |
...
| 17 | Run-Baler | Der auszuführende Befehl `Baler` für JavaScript-Bundling fehlgeschlagen | Überprüfen Sie die `SCD_USE_BALER` Umgebungsvariable, um zu überprüfen, ob das Baler-Modul für das JS-Bundling konfiguriert und aktiviert ist. Wenn Sie das Baler-Modul nicht benötigen, legen Sie `SCD_USE_BALER: false`. |
| 18 | compress-static-content | Erforderliches Dienstprogramm wurde nicht gefunden (Zeitüberschreitung, Bash) |  |
...
| 26 | validate-config | Entfernen Sie die Magento Braintree-Modulkonfiguration, die in Adobe Commerce und Magento Open Source 2.4 und höheren Versionen nicht mehr unterstützt wird. | Die Unterstützung für das Braintree-Modul ist in Commerce 2.4.0 und höher nicht mehr enthalten. Entfernen Sie die Variable CONFIG__STORES__DEFAULT__PAYMENT__BRAINTREE__CHANNEL aus dem Variablenabschnitt des `.magento.app.yaml` -Datei. Verwenden Sie stattdessen eine offizielle Erweiterung der Braintree-Zahlungsunterstützung von der Commerce Marketplace. |
...
| 102 |  | Schreiben in `./app/etc/env.php` file | Das Bereitstellungsskript kann die erforderlichen Änderungen nicht vornehmen `/app/etc/env.php` -Datei. Überprüfen Sie Ihre Dateisystemberechtigungen. |
...
| 107 | pre-deploy: clean-redis-cache | Redis-Cache konnte nicht gelöscht werden | Redis-Cache konnte nicht gelöscht werden. Überprüfen Sie, ob die Redis-Cache-Konfiguration korrekt ist und ob der Redis-Dienst verfügbar ist. Siehe [Setup Redis-Dienst](../services/redis.md). |
...
| 118 |  | Erforderliches Dienstprogramm wurde nicht gefunden (Zeitüberschreitung, Bash) |  |
...
| 121 | deploy-static-content:generate | Die bereitgestellte Version kann nicht aktualisiert werden | Die Aktualisierung der `./pub/static/deployed_version.txt` -Datei. Überprüfen Sie Ihre Dateisystemberechtigungen. |
...
| 136 | validate-config | Die Aufspaltungsdatenbank wurde ab Adobe Commerce und Magento Open Source 2.5.0 entfernt. | Wenn Sie eine geteilte Datenbank verwenden, die Sie wiederherstellen oder zu einer einzigen Datenbank migrieren müssen, oder einen alternativen Ansatz verwenden. |
...
| 202 |  | Die `./app/etc/env.php` Datei nicht schreibbar | Das Bereitstellungsskript kann die erforderlichen Änderungen nicht vornehmen `/app/etc/env.php` -Datei. Überprüfen Sie Ihre Dateisystemberechtigungen. |
...
| 207 | Warmup | Einige Seiten konnten nicht aktualisiert werden |  |
...
Warnungsfehler weisen auf ein Problem mit der Commerce-Konfiguration in Cloud-Infrastruktur-Projekten hin, z. B. falsche, veraltete, nicht unterstützte oder fehlende Konfigurationseinstellungen für optionale Funktionen, die sich auf den Site-Betrieb auswirken können. Obwohl eine Warnung keinen Bereitstellungsfehler verursacht, sollten Sie Warnmeldungen überprüfen und die Konfiguration aktualisieren, um sie zu beheben.
...
| 1004 | validate-config | Die Konfiguration ist nicht mit dieser Version von Commerce kompatibel |  |
...
| Fehler-Code | Bereitstellungsschritt | Fehlerbeschreibung (Titel) | Vorgeschlagene Aktion |
| - | - | - | - |
| 2001 | pre-deploy:cache | Der Cache ist für einen Redis-Dienst konfiguriert, der nicht verfügbar ist. Konfiguration wird ignoriert. |  |
...
| 2030 | validate-config | Elasticsearch- und OpenSearch-Dienste werden beide auf Infrastrukturebene installiert. Adobe Commerce und Magento Open Source 2.4.4 und höher verwenden OpenSearch standardmäßig. | Erwägen Sie, den Elasticsearch- oder OpenSearch-Dienst aus der Infrastrukturschicht zu entfernen, um die Ressourcennutzung zu optimieren. |
...
| 3004 | Backup | Sicherungsdateien können nicht erstellt werden |  |
