...
Ihre Adobe Commerce on Cloud Infrastructure Starter-Architektur unterstützt bis zu **vier** Umgebungen, einschließlich `master` Umgebung, die den anfänglichen Projektcode, die Staging-Umgebung und bis zu zwei Integrationsumgebungen enthält.
...
Sie können jede beliebige Entwicklungs- und Verzweigungsmethode verwenden. Wenn Sie anfänglichen Zugriff auf Ihr Projekt erhalten, erstellen Sie eine `staging` -Umgebung aus `master` Umgebung. Erstellen Sie dann die `integration` Umgebung durch Verzweigung von `staging`.
...
Da die `production` -Umgebung schreibgeschützt ist, verwenden Sie die `integration` -Umgebung, um Code-Änderungen vorzunehmen, stellen Sie sie in der gesamten Architektur über die `integration` nach `staging`und schließlich zum `production` Umgebung. Siehe [Bereitstellen Ihres Stores](../deploy/staging-production.md) und [Site-Launch](../launch/overview.md).

Adobe empfiehlt vollständige Tests in Ihrem `staging` Verzweigung vor dem Verschieben `master` -Verzweigung, die für die `production` Umgebung.

## Staging-Umgebung

Adobe empfiehlt die Erstellung einer Verzweigung mit dem Namen `staging` von `master`. Die `staging` -Verzweigung stellt Code in der Staging-Umgebung bereit, um eine Umgebung vor der Produktion bereitzustellen, in der Code, Module und Erweiterungen, Zahlungskanäle, Versand, Produktdaten und vieles mehr getestet werden können. Diese Umgebung bietet die Konfiguration für alle Dienste, die mit der Produktionsumgebung übereinstimmen, einschließlich Fastly, New Relic APM und Suche.
...
Entwickler verwenden die `integration` Umgebung zum Entwickeln, Bereitstellen und Testen:
...
Sie können **two** aktive Integrationsumgebungen. Sie erstellen eine Integrationsumgebung, indem Sie eine Verzweigung aus dem `staging` -Verzweigung. Wenn Sie eine Integrationsumgebung erstellen, entspricht der Umgebungsname dem Zweignamen. Eine Integrationsumgebung umfasst einen Webserver und eine Datenbank. Sie umfasst nicht alle Dienste, z. B. Fastly CDN und New Relic sind nicht verfügbar.
...
Die Produktions- und Staging-Umgebungen umfassen die folgenden Technologien. Sie können diese Technologien über die [`.magento.app.yaml`](../application/configure-app-yaml.md) -Datei.
...
Sie können eine Sicherung Ihrer Datenbank und Ihres Dateisystems mithilfe der Variablen [!DNL Cloud Console] oder die CLI. Siehe [Backup-Management](../storage/snapshots.md).
...
1. Klonen Sie die `master` Verzweigung in der lokalen Umgebung

1. Erstellen Sie eine `staging` Verzweigung aus `master`

1. Erstellen von Zweigen für die Entwicklung aus `staging`
